...
# Формат пакета: W = 32x6 = 192 float. B = 6 float. Итого 198 float.
# '<' означает little-endian (стандартно для ESP32). '198f' - 198 чисел типа float (по 4 байта = 792 байт)
PAYLOAD_FORMAT = '<198f'
...
def on_message(client, userdata, msg):
    try:
        ...
        if len(msg.payload) == struct.calcsize(PAYLOAD_FORMAT):
            ...
            # Извлекаем W (первые 192 элементов) и восстанавливаем размерность 32x6
            W = np.array(data[:192]).reshape((32, 6))
            # Извлекаем B (последние 6 элементов)
            B = np.array(data[192:])
